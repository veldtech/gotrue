-- Multi-instance mode (see instances) table intentionally not supported and ignored.
...
create unique index if not exists sso_providers_resource_id_idx on sso_providers (lower(resource_id));
...
create unique index if not exists sso_domains_domain_idx on sso_domains (lower(domain));
...
create index if not exists sso_sessions_sso_provider_id_idx on sso_sessions (sso_provider_id);
comment on table sso_sessions is 'Auth: A session initiated by an SSO Identity Provider';