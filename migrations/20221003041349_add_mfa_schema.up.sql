...
do $$ begin create type factor_type as enum('totp', 'webauthn');
create type factor_status as enum('unverified', 'verified');
create type aal_level as enum('aal1', 'aal2', 'aal3');
exception
when duplicate_object then null;
end $$;
-- mfa_factors definition
...
create table if not exists mfa_challenges(
    ...
    constraint mfa_challenges_auth_factor_id_fkey foreign key (factor_id) references mfa_factors(id) on delete cascade
);
...
create table if not exists mfa_amr_claims(
    ...
    constraint mfa_amr_claims_session_id_fkey foreign key(session_id) references sessions(id) on delete cascade
);
