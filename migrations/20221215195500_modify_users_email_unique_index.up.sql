...
do $$ begin
alter table only users drop constraint if exists users_email_key;
exception -- dependent object: https://www.postgresql.org/docs/current/errcodes-appendix.html
when SQLSTATE '2BP01' then raise notice 'Unable to drop users_email_key constraint due to dependent objects, please resolve this manually or SSO may not work';
end $$;
create unique index if not exists users_email_partial_key on users (email)
